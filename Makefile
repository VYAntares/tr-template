...
#
# `make run` is the launch-only slice of that, no checks: what someone needs to
# see the app running. `make dev` is the hot-reload development stack.
...
DATABASE_URL ?= postgresql://ft:ft_local_dev@$(DB_HOST):5432/template_app?schema=public
...
DEFAULT_DATABASE_URL := postgresql://ft:ft_local_dev@db:5432/template_app?schema=public
ifneq ($(wildcard .env),)
DB_ENV := --env-file .env
else
...
test-e2e: ## Playwright against the production stack (starts it if needed)
	...
	@# repeated runs from filling the database with throwaway rows.
	...
	status=$$?; \
	$(COMPOSE_DEV) exec -T db psql -U ft -d template_app \
...
seed: ## Load development data into the database
	@if ! grep -q '"db:seed"' apps/api/package.json; then \
	  ...
	  printf 'seed: add prisma/seed.ts and a "db:seed" script to enable this.\n'; \
	  exit 0; \
	fi; \
	$(MAKE) --no-print-directory tooling-image; \
	docker run --rm --network $(NETWORK) $(DB_ENV) $(TOOLING_IMAGE) \
...
certs: ## Export Caddy's local root CA to ./certs
	...
	printf 'Trust it in a browser (the first two need NO sudo):\n'; \
	...
	printf 'From another device (e.g. your laptop, stack on another machine):\n'; \
	printf '  add "<server-ip> %s" to /etc/hosts there, import the cert, open https://%s\n' \
	  "$${CADDY_EXTRA_HOSTNAME:-template-app.test}" "$${CADDY_EXTRA_HOSTNAME:-template-app.test}"
