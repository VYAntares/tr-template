...
| `redis` (node-redis) | 6.2.0 | Official client. Used for the throttler store. |
| BullMQ | 6.0.5 | |
...
The image ships pgvector, which this template does not use yet. It costs nothing
to keep and saves a database migration later if you ever add embeddings; swap it
for the plain `postgres` image if you are sure you never will. If you do keep it,
pgvector must be >= 0.8.2: CVE-2026-3172, buffer overflow in parallel HNSW index
builds. The plain `postgres` image does **not** include pgvector.
