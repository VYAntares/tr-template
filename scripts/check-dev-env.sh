...
printf '%stemplate_app development environment%s\n' "$B" "$Z"
...
# --- git identity ------------------------------------------------------------
# An unset identity fails silently: the commit still lands, just attributed to
# nobody recognisable, and it never links back to a GitHub account.
...
if [ -n "$git_name" ] && [ -n "$git_email" ]; then
  ...
  hint 'This address must be on your GitHub account, or your commits are not'
  hint 'linked to you anywhere in the GitHub UI.'
else
