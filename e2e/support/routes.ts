 ...
 *
 * The last two are surfaces people actually open: /api/docs is printed by `make`
 * and is third-party Swagger UI whose console output is not ours, and the 404
 * page is rendered by Next but is otherwise only checked over HTTP, so nothing
 * would watch its console.
 */
...
/**
 * The legal pages must exist and carry real text. Mirrors
 * LEGAL_MINIMUM_CHARACTERS in apps/web/lib/legal.ts, which a unit test already
 * asserts on the source data. This is the same floor measured on the rendered
 * page, so a routing or layout regression cannot hide behind a passing unit
 * test.
 */
